...
import { buildAssignedToMeWhere, getAccessibleDepartmentSummaries } from "@/lib/services/department-scope-service";
...
function buildCanonicalUrl(params: SearchParams, page: number): string {
  ...
  return `/tickets/assigned-to-me?${canonical.toString()}`;
}

/**
 * Tickets currently assigned to the current user (assignedAgentId) — split
 * out of the old "My Tickets" (which only ever checked requesterId, never
 * assignment) alongside /tickets/created-by-me. No department-membership
...
export default async function AssignedToMeTicketsPage({
  ...
  // just narrows an already-personal result set — see buildAssignedToMeWhere
